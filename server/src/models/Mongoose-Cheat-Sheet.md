## Comprehensive Mongoose Cheat Sheet
...
newUser
  .save()
  .then(() => console.log("User created")) // Log a success message upon successful saving
  .catch((err) => console.error(err)); // Log any errors that occur during saving
```
...
User.find()
  .then((users) => console.log(users)) // Log the array of retrieved users
  .catch((err) => console.error(err)); // Log any errors that occur during retrieval
```
...
User.findById("user_id_here") // Replace 'user_id_here' with an actual user ID
  .then((user) => console.log(user)) // Log the retrieved user document
  .catch((err) => console.error(err)); // Log errors if the retrieval fails
```
...
User.find({ age: { $gt: 25 } }) // Use the $gt (greater than) operator to filter users by age
  .then((users) => console.log(users)) // Log the array of users that match the condition
  .catch((err) => console.error(err)); // Log any errors that occur during retrieval
```
...
User.updateOne({ _id: "user_id_here" }, { age: 31 }) // Specify the user ID and the update
  .then(() => console.log("User updated")) // Log a success message upon successful update
  .catch((err) => console.error(err)); // Log any errors that occur during the update
```
...
User.findByIdAndUpdate("user_id_here", { name: "Jane Doe" }, { new: true }) // The { new: true } option returns the updated document
  .then((updatedUser) => console.log(updatedUser)) // Log the updated user document
  .catch((err) => console.error(err)); // Log errors if the update fails
```
...
User.deleteOne({ _id: "user_id_here" }) // Specify the ID of the user to delete
  .then(() => console.log("User deleted")) // Log a success message upon successful deletion
  .catch((err) => console.error(err)); // Log any errors that occur during deletion
```
...
User.findByIdAndDelete("user_id_here") // Replace 'user_id_here' with an actual user ID
  .then((deletedUser) => console.log(deletedUser)) // Log the deleted user document
  .catch((err) => console.error(err)); // Log errors if the deletion fails
```
...
// Usage example
User.findByEmail("john.doe@example.com") // Call the custom query helper
  .then((user) => console.log(user)) // Log the found user document
  .catch((err) => console.error(err)); // Log errors if the query fails
```
...
allows you to reference documents in other collections. This is useful for establishing relationships between different models.
...
Post.find()
  ...
  .exec((err, posts) => {
    if (err) return console.error(err); // Handle error if any
...
Creating indexes on certain fields can improve query performance significantly. Here’s how to create an index on a field in a schema:
...
User.aggregate([
  ...
  .then((result) => console.log(result)) // Log the aggregation result, showing counts of users by age
  .catch((err) => console.error(err)); // Log errors if the aggregation fails
```

### Error Handling

Proper error handling is crucial for maintaining application stability. Always handle errors using try-catch blocks or the `.catch()` method with promises.
